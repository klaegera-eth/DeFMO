...
time = datetime.now()
filename = time.strftime(f"{len(p['blurs'])}-{p['n_frames']}_%y%m%d%H%M%S.zip")
...
with zipfile.ZipFile(os.path.join(out_dir, filename), "w") as zip:
    ...
    for i in range(n_sequences):
        ...
        with zip.open(f"{i:04}.webp", "w") as out:
            with objs.get_random_path() as obj, texs.get_random_path() as tex:
                render.render(
                    out,
                    obj,
                    tex,
                    frustum.gen_point_pair(p["delta_z"], p["delta_xy"]),
                    (rot_start, rot_end),
                    p["blurs"],
                )
...
print(
    f"Rendered {n_sequences} sequences in {duration}, {duration.total_seconds() / n_sequences:.2f}s per sequence"
)
