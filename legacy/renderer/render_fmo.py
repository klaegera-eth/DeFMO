...
def renderTraj(pars, H):
    ...
    for timeinst in range(rangeint.shape[0] - 1):
        ...
        valid = np.logical_and(
            np.logical_and(rr < H.shape[0], cc < H.shape[1]), np.logical_and(rr > 0, cc > 0)
        )
        rr = rr[valid]
        cc = cc[valid]
        val = val[valid]
    ...
    while tri <= g_max_trials:
        ...
        if not g_apply_texture:
            for oi in range(len(bpy.data.objects)):
                if bpy.data.objects[oi].type in ("CAMERA", "LAMP", "LIGHT"):
                    continue
        ...
        los_start = Vector(
            (
                random.uniform(-maxlen / 10, maxlen / 10),
                random.uniform(-maxlen, maxlen),
                random.uniform(-maxlen, maxlen),
            )
        )
        loc_step = (
        ...
        for ki in [0, ni - 1] + list(range(1, ni - 1)):
            ...
            if ki == 0 or ki == (ni - 1):
                Mt = (
                    cv2.imread(
                        os.path.join(
                            bpy.context.scene.node_tree.nodes[1].base_path,
                ...
                is_border = (
                    (Mt[0, :].sum() + Mt[-1, :].sum() + Mt[:, 0].sum() + Mt[:, -1].sum()) > 0
                ) or Mt.sum() == 0
                if is_border:
                    if ki == 0:
                        close_log(old)
...
def make_fmo(path, gt_path, video_path):
    ...
    B = cv2.resize(
        B0,
        dsize=(
            int(g_resolution_x * g_resolution_percentage / 100),
            int(g_resolution_y * g_resolution_percentage / 100),
        ),
    ...
    for ki in range(g_fmo_steps):
        FM[:, :, :, ki] = (
            cv2.imread(
                os.path.join(gt_path, "image-{:06d}.png".format(ki + 1)), cv2.IMREAD_UNCHANGED
            )
            / g_rgb_color_max
        )
    ...
    Image.fromarray((B[:, :, [2, 1, 0]] * 255).astype(np.uint8)).save(
        os.path.join(gt_path, "bgr.png")
    )
    Image.fromarray((Bmed[:, :, [2, 1, 0]] * 255).astype(np.uint8)).save(
        os.path.join(gt_path, "bgr_med.png")
    )
    ...
    if False:
        ...
        out = cv2.VideoWriter(
            video_path, cv2.VideoWriter_fourcc(*"MJPG"), 6, (F.shape[1], F.shape[0]), True
        )
        for ki in range(g_fmo_steps):
            out.write(Fwr[:, :, :, ki])
        out.release()
...
def render_obj(obj_path, path, objid, obj_name, temp_folder):
    ...
    if g_apply_texture:
        for oi in range(len(bpy.data.objects)):
            if bpy.data.objects[oi].type in ("CAMERA", "LAMP", "LIGHT"):
                continue
            bpy.context.view_layer.objects.active = bpy.data.objects[oi]
            # pdb.set_trace()
            ...
            for mat in bpy.data.materials:
                ...
                textureNode.image = bpy.data.images.load(tex_path)
                links.new(
                    nodes["Principled BSDF"].inputs["Base Color"], textureNode.outputs["Color"]
                )
            # print(bpy.data.objects[oi].active_material)
